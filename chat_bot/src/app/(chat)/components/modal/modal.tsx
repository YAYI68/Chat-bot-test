...
export function ChatModal () {
    const router = useRouter()
     const {setIsModal, modalContent} = useChatManager()
     const { sendChat } = useDeleteChat({
        onSuccess: ({ data: { data } }) => {
        },
        onError: () => {
      ...
      function handleSubmit() {
              void sendChat({
                  chatId: modalContent?.chatId as string,
                });
                setIsModal(false)
                router.push('/')
    ...
    return <div className="w-screen h-screen fixed  inset-0 backdrop-blur-xs backdrop-brightness-90 z-50 left-0 top-0">
      <div className="w-full h-full flex flex-col items-center justify-center">
          <div className="w-[80%] lg:w-[27rem] h-fit p-8 rounded-lg bg-primary-200 flex flex-col gap-4">
            <p className="text-label-500">Are you sure you want to delete Conversation {modalContent.num}?</p>
